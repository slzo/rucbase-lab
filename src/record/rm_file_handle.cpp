...
RmPageHandle RmFileHandle::fetch_page_handle(int page_no) const {
    ...
    return RmPageHandle(&file_hdr_, buffer_pool_manager_->FetchPage(page_id));
}
 ...
 * @return RmPageHandle
 */
RmPageHandle RmFileHandle::create_new_page_handle() { //ok
    // Todo:
    // 1.使用缓冲池来创建一个新page
    ...
    PageId page_id;
    page_id.fd = fd_;
    Page *page = buffer_pool_manager_->NewPage(&page_id); // 1
