...
但是可能会出现select record非常多,而limitvalue非常小的情况,此时会浪费大量的时间做无用的排序,对此可以采用堆排序的方式来改进,具体实现代码并未实现
...
由于是第一年设计并采用rucbase-lab作为课程实验,该实验还存在一些问题
...
总体来说,实验的目的还是达到了一部分,通过实验确实是对数据库的底层实现有着更加深入的了解,考虑到是第一次设计和使用该实验,整体的设计还是很新颖的,相信经过不断地完善和改变,该实验会越来越好的.
